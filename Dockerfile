...
RUN jar uf target/leihs-my.jar resources/built-info.yml
...
COPY --from=builder ${WORKDIR}/target/leihs-my.jar target/
...
# run
EXPOSE ${HTTP_PORT}
ENTRYPOINT [ "java", "-jar", "target/leihs-my.jar", "run" ]