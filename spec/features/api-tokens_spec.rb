  ...
  context 'signed in as an admin' do
    ...
    context 'an API-Token for the current user has been added' do
      ...
      let :auth_info_response_without_token do
        plain_faraday_json_client.get('/user/me/auth-info')
      end

      let :auth_info_response_with_token_auth do
        plain_faraday_json_client.get('/user/me/auth-info') do |conn|
      ...
      scenario 'the token can be used to ' \
        ...
        expect(plain_faraday_json_client.get('/user/me/auth-info').status).to be== 401

        expect(
          plain_faraday_json_client.get('/user/me/auth-info'){|conn|
            conn.headers['authorization'] = "token #{@token_secret}"
          }.status).to be== 200
      end
      ...
      scenario 'the token can be used to ' \
        ...
        expect(plain_faraday_json_client.get('/user/me/auth-info').status).to be== 200
      end
